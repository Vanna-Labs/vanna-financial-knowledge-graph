...
Phase 3 of the VannaKG ingestion pipeline transforms resolved entities, topics, and extracted facts into a complete knowledge graph stored in the embedded runtime (Parquet + DuckDB + LanceDB). This phase receives the outputs of Phase 1 (extraction) and Phase 2 (resolution/deduplication) and performs three critical sub-phases:
...
VannaKG uses a **chunk-centric fact pattern** where relationships flow through the EpisodicNode (chunk) that contains them. This design enables precise source attribution - every fact can be traced back to its exact source chunk.
