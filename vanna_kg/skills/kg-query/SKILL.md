---
name: kg-query
description: Use when issuing VannaKG MCP `kg_execute` shell-style commands to resolve names (`find`), search graph connections (`search`), inspect fact details (`cat`), and perform temporal/provenance analysis over financial-document knowledge bases.
---

# VannaKG Graph Query Skill
