# Python Package Design: vanna-kg

## Executive Summary

### Vision

Transform VannaLabsKG from an infrastructure-dependent knowledge graph system into a **pip-installable Python library** with zero server requirements. Users should be able to run:
...
# Bundles
all-providers = ["vanna-kg[openai,anthropic,google,voyage]"]
all = ["vanna-kg[all-providers,rust]"]
...
class KGConfig:
    ...
    3. Config file (~/.vanna/config.toml or specified path)
    4. Built-in defaults
...
# All cloud providers
cloud = [
    "vanna-kg[openai,anthropic,google,voyage]",
]
...
|--------------|---------------------|---------|
| `llm_provider` | `VANNA_LLM_PROVIDER` | `anthropic` |
| `llm_model` | `VANNA_LLM_MODEL` | `claude-sonnet-4-5` |
| `embedding_provider` | `VANNA_EMBEDDING_PROVIDER` | `voyage` |
| `openai_api_key` | `OPENAI_API_KEY` | `sk-...` |
| `anthropic_api_key` | `ANTHROPIC_API_KEY` | `sk-ant-...` |
| `extraction_concurrency` | `VANNA_EXTRACTION_CONCURRENCY` | `20` |

**API key precedence:** Provider-specific environment variables (e.g., `OPENAI_API_KEY`) are checked before `VANNA_*` variants for compatibility with existing setups.
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install vanna-kg[local-embed]"
            )
...
```bash
# Ingestion
vanna-kg ingest document.pdf --kb ./my_kb
vanna-kg ingest ./docs/ --pattern "*.md" --kb ./my_kb

# Query
vanna-kg query "What were the key risks?" --kb ./my_kb
vanna-kg query --interactive --kb ./my_kb
...
vanna-kg info --kb ./my_kb --entities
vanna-kg info --kb ./my_kb --documents

# Export
vanna-kg export --format json --output ./export.json --kb ./my_kb
vanna-kg export --format rdf --output ./export.ttl --kb ./my_kb

# Migration
vanna-kg migrate-from-neo4j --uri bolt://... --user neo4j --kb ./my_kb
```

### vanna-kg ingest

```bash
vanna-kg ingest [OPTIONS] PATHS...
...
Examples:
  ...
  vanna-kg ingest ./documents/ --kb ./my_kb

  # Ingest markdown files
  vanna-kg ingest ./notes/ --pattern "**/*.md" --kb ./my_kb

  # Resume interrupted ingestion
  vanna-kg ingest ./large_corpus/ --kb ./my_kb --resume
```

### vanna-kg query

```bash
vanna-kg query [OPTIONS] [QUESTION]
...
Examples:
  # Single question
  vanna-kg query "What economic conditions were reported?" --kb ./my_kb

  # Interactive mode
  vanna-kg query -i --kb ./my_kb

  # JSON output for scripting
  vanna-kg query "List all companies mentioned" --kb ./my_kb --json

  # With shell-based exploration
  vanna-kg query --shell "How are Apple and Google connected?" --kb ./my_kb
```

### vanna-kg shell

```bash
vanna-kg shell [OPTIONS]
...
Examples:
  ...
  # JSON output
  vanna-kg info --kb ./my_kb --json --stats
```
...
[project.urls]
Homepage = "https://github.com/Vanna-Labs/vanna-kg"
Documentation = "https://vanna-kg.readthedocs.io"
Repository = "https://github.com/Vanna-Labs/vanna-kg"
Issues = "https://github.com/Vanna-Labs/vanna-kg/issues"
```
...
Users with existing Neo4j-based VannaLabsKG deployments can migrate to the embedded architecture.

**Migration command:**
```bash
vanna-kg migrate-from-neo4j \
...
```python
# Before (Neo4j-based)
from vanna_kg import Pipeline, QueryEngine
...
| Environment variables | Same, plus `VANNA_*` variants |
...
**Legacy module:**
```python
# vanna_kg/_compat/legacy.py

# Import old API names for backwards compatibility
from vanna_kg import KnowledgeGraph as Pipeline
from vanna_kg import KnowledgeGraph
...
This document describes the design for transforming VannaLabsKG into a pip-installable Python package with zero infrastructure requirements. Key architectural decisions:
