...
Environment Variables:
    ZOMMA_LLM_PROVIDER - LLM provider name
    ZOMMA_LLM_MODEL - Model for extraction/synthesis
    ZOMMA_EMBEDDING_PROVIDER - Embedding provider name
    ZOMMA_EXTRACTION_CONCURRENCY - Max concurrent extraction LLM calls
    ZOMMA_REGISTRY_CONCURRENCY - Max concurrent entity registry resolutions
...
class KGConfig:
    ...
    def _load_from_env(self) -> None:
        ...
        # ZOMMA_* prefixed settings
        if provider := os.getenv("ZOMMA_LLM_PROVIDER"):
            self.llm_provider = provider
        if model := os.getenv("ZOMMA_LLM_MODEL"):
            self.llm_model = model
        if provider := os.getenv("ZOMMA_EMBEDDING_PROVIDER"):
            self.embedding_provider = provider
        if concurrency := os.getenv("ZOMMA_EXTRACTION_CONCURRENCY"):
            self.extraction_concurrency = int(concurrency)
        if concurrency := os.getenv("ZOMMA_REGISTRY_CONCURRENCY"):
