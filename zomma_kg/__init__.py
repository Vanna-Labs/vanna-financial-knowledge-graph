"""
ZommaKG - Embedded Knowledge Graph Library
...
def __getattr__(name: str):
    ...
    if name in ("ingest_pdf", "ingest_markdown", "ingest_chunks", "query", "decompose"):
        from zomma_kg.api import convenience
        return getattr(convenience, name)

    # Types
    if name in ("Chunk", "Entity", "Fact", "Topic", "Document", "QueryResult"):
        from zomma_kg import types
        return getattr(types, name)

    raise AttributeError(f"module 'zomma_kg' has no attribute {name!r}")
